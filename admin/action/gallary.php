    ...
    $date = date("Y-m-d h:i:s");
        if (isset($_POST['check']) && $_POST['check'] != "") {
          ...
          if (mysqli_num_rows($result) > 0) {
            ...
            //print_r($_POST);
          $queryUpdate = mysqli_query($connect,"UPDATE gallary SET cat_name = '$cat_name',image = '$image',dt_updated = '$date' WHERE id = '$id'");
          }
          header('location: '.$location.'admin/gallary.php'); 
        }
         else{
          $cat_name = $_POST['cat_name'];
    $image = null;
    if(isset($_FILES["image"]["name"]) && !empty($_FILES["image"]["name"]))
    {
      ...
      { 
        ...
        else
        {
          ...
          $query=mysqli_query($connect,"INSERT INTO gallary (cat_name,image,dt_created) VALUES('$cat_name','$image','$date')");
        }
    header('location: '.$location.'admin/gallary.php'); 
  } 
?>
